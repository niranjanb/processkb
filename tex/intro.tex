...
In particular, we propose to build a semantic-role based knowledge base of scientific concepts involving simple physical, chemical, and other natural phenomena. Standardized test benchmarks such as grade-level science exams~\cite{clark2015elementary} motivate automatic knowledge extraction and reasoning~\cite{clark2014:akbc,chb2013:akbc, khot2015:emlnlp}. 
...
%To this end, the reasoner must first assemble a representation of the key bits of the information:
%1) The thing being heated, the {\em undergoer}, is the pot. 
%2) The  {\em result} is that the pot handle becomes hot.
%The next step is to verify if the result in the scenario is plausible given the following bits of knowledge about heating and conduction processes:
%1) The result of heating is that the thing being heated (the pot) becomes hot.

%Further, to perform effective reasoning a system must chain together few pieces of information some of which has to be inferred:
%1) The {\em result} of heating is that the pot becomes hot. 
%2) In a conduction process heat is transferred via direct contact.
%3) The handle is part of the pot and is thus in direct contact with the pot. 
...
%To apply these background knowledge we must be able to identify {\em what is being heated} and {\em what has become hot} in the scenario.
